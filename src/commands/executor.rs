...
use crate::server::cluster::{key_slot, ClusterManager, SlotCheck};
...
const SKIP_SLOT_CHECK: &[&str] = &[
    ...
    // Connection commands
    "PING", "ECHO", "AUTH", "HELLO", "CLIENT", "SELECT", "QUIT", "RESET",
    // Server commands
    "INFO", "DEBUG", "CONFIG", "COMMAND", "MEMORY", "MODULE", "SLOWLOG", "ACL", "LATENCY",
    "DBSIZE", "LASTSAVE", "TIME", "BGSAVE", "BGREWRITEAOF", "SAVE", "SHUTDOWN", "FLUSHALL", "FLUSHDB",
    ...
    "KEYS", "SCAN", "OBJECT", "WAIT", "WAITAOF", "DEBUG",
];
...
impl CommandExecutor {
    ...
    pub fn with_cluster(database: Arc<Database>, pubsub: SharedPubSubHub, cluster: Arc<ClusterManager>) -> Self {
        Self {
            registry: CommandRegistry::new(),
            database,
            pubsub: Some(pubsub),
    ...
    pub async fn execute(
        ...
        trace!("Executing command: {} with {} args", cmd.name, cmd.arg_count());
        ...
        // Look up command
        let command = self.registry.get(&cmd.name).ok_or_else(|| {
            Error::Command(CommandError::UnknownCommand(cmd.name.clone()))
        })?;
                ...
                if let Some(key) = self.get_first_key(&cmd, command) {
                    ...
                    match cluster.check_slot_number(slot) {
                        ...
                        SlotCheck::Moved { slot, addr } => {
                            ...
                            return Ok(Frame::Error(format!("MOVED {} {}:{}", slot, addr.ip(), addr.port())));
                        }
                        SlotCheck::Ask { slot, addr } => {
                            // Migration in progress - client should send ASKING first
                            if !is_asking {
                                return Ok(Frame::Error(format!("ASK {} {}:{}", slot, addr.ip(), addr.port())));
                            }
    ...
    async fn execute_transaction(&self, client: Arc<ClientState>) -> Result<Frame> {
        ...
        for queued in commands {
            ...
            match self.registry.get(&cmd.name) {
                ...
                None => {
                    results.push(Frame::error(format!(
                        "ERR unknown command '{}'",
                        cmd.name
                    )));
                }
    ...
    async fn handle_publish(&self, cmd: &ParsedCommand) -> Result<Frame> {
        ...
        let pubsub = self.pubsub.as_ref().ok_or_else(|| {
            Error::Internal("Pub/Sub not initialized".to_string())
        })?;
    ...
    async fn handle_pubsub_command(&self, cmd: &ParsedCommand) -> Result<Frame> {
        ...
        match subcommand.as_str() {
            ...
            "NUMPAT" => {
                Ok(Frame::Integer(pubsub.numpat() as i64))
            }
    ...
    async fn handle_auth(&self, cmd: &ParsedCommand, client: Arc<ClientState>) -> Result<Frame> {
        ...
        if password_valid {
            ...
            if locked_out {
                Ok(Frame::Error(
                    "ERR too many authentication failures, account locked for 60 seconds".to_string()
                ))
            } else {
                Ok(Frame::Error("WRONGPASS invalid username-password pair".to_string()))
            }
        }
    }
